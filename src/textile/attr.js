...
/**
 * Copy a shallow copy of the given object, excluding any keys given in the
 * blacklist.
 *
 * @param {Object} s - The object to copy.
 * @param {Object} blacklist - The keys to exclude from the copy.
 *
      ...
      if (
        ...
        for (const bit of bits) {
          if (bit.startsWith("*")) {
            ...
            if (element === "bc") {
              o.class = o.class
                ? `${o.class} language-${lang.shift().trim()}`
                : `language-${lang.shift().trim()}`;
            } else {
              o.class = o.class ? `${o.class} ${bit}` : bit;
            }
          } else if (bit.startsWith("#")) {
            // h1(foo #bar biz #foobiz) will return the first one bar
            if (!o.id) {
              o.id = bit.slice(1);
            }
          } else if (bit.includes("#")) {
            // solve original textile syntax for class and id , like h1(class#id). ..
            const sbits = bit.split("#");
            o.class = o.class ? `${o.class} ${sbits[0]}` : sbits[0];
            if (sbits[1] && !o.id) {
              o.id = sbits[1];
            }
          } else {
